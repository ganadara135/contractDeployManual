...
web3.eth.getTransactionCount(addressMy, (err, txCount) => {
    ...
    const txObject = {
      ...
      to: contractAddress,
      data: myContract.methods.getInstructor().encodeABI()
    }
...
myContract.methods.getInstructor().call((err, result, result2) => {
...
let blockinfoFunc =  async function() {
    ...
    let reValue  =  await myContract.methods.getInstructor().call((err, result, result2) => {
                    console.log({ err , balance})
                    console.log(" err : ", err)
                    console.log(" name : ",  result)
                    console.log( " age : ", result2)
